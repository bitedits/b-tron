...
\begin{itemize}[leftmargin=*]
    ...
    \item \texttt{TS\_VOBJ}: Embedded Virtual Object pointer link.
\end{itemize}
...
\begin{table}[htbp]
\caption{Logical Structure of a NITRO/FORM Real Object}
...
|  * Display Primitives (DP)  * TAD + Real Objects  |
+---------------------------------------------------+
|                   uITRON Kernel                   |
+---------------------------------------------------+
\end{lstlisting}
...
\midrule
\texttt{<article>}, \texttt{<form>} & Top-level Form TAD Real Object \\
...
\begin{lstlisting}
  +-------------------------------------------------+
  | 1. Open Form TAD (Resolve Real Object Graph)    |
  +------------------------+------------------------+
                           |
                           v
  +-------------------------------------------------+
...
%-------------------------------------------------------------------------
\section{Implementation Report: The Native TAD Document Browser and Real Object Cabinet Explorer}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Binary Segment Parsing:} Scans standard TAD segment records ($T_{\text{tag}}$, $L_{\text{len}}$) including \texttt{TS\_TEXT} (multi-lingual TRON Code character sequences), \texttt{TS\_FHR} (horizontal vector rules), \texttt{TS\_VOBJ} (Virtual Object link descriptors with real object ID target and label), and \texttt{TS\_FPRIM} (figure/picture segments).
...
\subsection{Dynamic Real Object Cabinet Explorer}
The Real Object Cabinet Explorer provides an interactive, window-based filing system replacing static catalog listings with dynamic filesystem discovery:
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Multi-Byte Alignment Immunity:} Renders discrete data fields (catalog tag, TOC hierarchy path, real object ID, document title, file byte size) at fixed horizontal pixel offsets, completely eliminating column misalignment caused by variable-width UTF-8 Cyrillic and Japanese Kanji glyphs.
\end{enumerate}
...
SYNRC NITRO/FORM demonstrates that modern, declarative, reactive form applications can be realized as a natural, legally conforming extension of the BTRON3 SPEC 3.20 standard. By encapsulating field schemas within application-range fusen tags, composing atomic controls via the Parts Manager, and maintaining data bindings as first-class Real Objects, the framework establishes a highly productive business application environment without requiring core kernel modifications.
