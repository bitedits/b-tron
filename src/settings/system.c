...
static void paint_system_settings(WND *wnd, GDEV *dev) {
    ...
    draw_setting_gif_icon(dev, "system", 6, 6);
    char hdr_str[128];
    snprintf(hdr_str, sizeof(hdr_str), "[Settings Cabinet] %s (%s) - %s", "System", "基本情報", "System Plane info, memory & SMP");
    ...
    drw_rec(dev, &s1);
    drw_tc_string(dev, 16, 30, " [1. System Plane & Kernel Information] ", COLOR_NAVY, COLOR_WHITE);
    paint_ui_checkbox(dev, 18, 52, "B-System 3.20 (BTRON3 Cleanroom Implementation)", g_state_system.checks[0], FALSE);
    paint_ui_checkbox(dev, 18, 74, "Multi-Arch CPU Support (ARMv7, x86_64, PC-98)", g_state_system.checks[1], FALSE);
    paint_ui_checkbox(dev, 18, 96, "SMP Multiprocessing: 4 Active Preemptive Cores", g_state_system.checks[2], FALSE);
    paint_ui_checkbox(dev, 18, 118, "Memory Status: 1024 MB Physical RAM (874 MB Free)", g_state_system.checks[3], FALSE);
