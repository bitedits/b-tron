...
where:
\begin{itemize}[leftmargin=*]
    \item $\mathcal{T}_{\text{Target}} \in \{\text{Switch}, \text{Selector}, \text{Volume}\}$,
    \item $\mathcal{M}_{\text{State}} \in \{\text{Momentary (\ja{モーメンタリ})}, \text{Alternate (\ja{セットアップ})}\}$,
    \item $\mathcal{A}_{\text{Action}} \in \{\text{Push}, \text{Pull}, \text{Push-Pull}, \text{Slide}, \text{Rotary}, \text{See-saw}, \text{Tilt}\}$.
\end{itemize}
...
\begin{equation}
[\text{Cancel / \ja{取り消し}}] \longrightarrow [\text{Default / \ja{標準設定}}] \longrightarrow [\text{Start / \ja{実行}}]
\end{equation}
\begin{itemize}[leftmargin=*]
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Three-Color Chromatic Protocol (Chapter 9):} $\mathcal{S}_{\text{LED}} \in \{\text{Green (Active/Safe)}, \text{Amber (Standby/Transit)}, \text{Red (Stop/Error)}\}$.
...
\begin{equation}
\kappa_{t+1} = (\kappa_t \pm 1) \bmod n, \quad v(\kappa)_{t+1} = \operatorname{clamp}(v(\kappa)_t \pm \delta, v_{\min}, v_{\max})
\end{equation}
