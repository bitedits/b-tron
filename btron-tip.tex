...
\textbf{User Dictionary} & SQLite / Binary file on local disk & BTRON Real Object (\ja{実身}) Persistent Cabinet Records \\
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Group 5: Real Object User Dictionary Learning:} Validates persistent storage, runtime registration of custom vocabulary, file export/import, and adaptive frequency score boosting.
...
1. Romaji / Kana Transliteration (F6--F9) & 7 & 100.0\% \\
2. Viterbi Lattice Bunsetsu Segmentation & 6 & 100.0\% \\
3. Candidate Ranking \& Semantic Tags & 5 & 100.0\% \\
4. DFA State Transitions \& $O(1)$ Escape & 5 & 100.0\% \\
5. User Dictionary Real Object Learning & 5 & 100.0\% \\
6. Lossless TRON Code / UTF-8 Bridge & 4 & 100.0\% \\ \midrule
\textbf{Total Test Suite Coverage} & \textbf{32 / 32} & \textbf{100.0\%} \\ \bottomrule
\end{tabular}
}
\end{table}

As summarized in Table~\ref{tab:test_results}, the test suite achieves a 100.0\% pass rate across all 32 core assertions, verifying both theoretical safety invariants and practical linguistic conversion accuracy.

%-------------------------------------------------------------------------
\section{Real Object User Dictionary Architecture}
\label{sec:real_objects}

In conventional Unix environments, user dictionaries are maintained as unmanaged flat files in hidden dotfiles (\texttt{\textasciitilde/.config/mozc/user.dic}). In B-System, user dictionaries are modeled as first-class **Real Objects (\ja{実身} -- \emph{Jisshin})**:
\begin{itemize}[leftmargin=*]
    \item \textbf{Hyper-Data Integration:} The user dictionary is stored as an immutable-header record in the system cabinet. Any application document can link to specialized dictionaries (e.g., medical, legal, avionics terminology) via Virtual Objects (\ja{仮身} -- \emph{Kashin}).
