...
We conclude that the optimal compromise for B-System 3.20 is not a purist user-space microkernel, but a \textbf{Capability-Gated Ring-Partitioned Architecture}. In this model:
\begin{enumerate}[leftmargin=*]
    \item The Core Kernel, Local APIC interrupt dispatchers, and high-speed Display Primitives (DP) rendering engine reside in Ring~0.
    \item User Applications, the Hierarchical File/Data Set (HFDS) storage daemon, and network protocol stacks execute in isolated Ring~3 process address spaces.
    \item IPC between Ring~3 processes and the kernel leverages the zero-switch Erlang-style MPSC lock-free queues established in Section~\ref{sec:erlang}.
\end{enumerate}
